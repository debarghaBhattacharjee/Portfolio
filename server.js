...
app.post('/register', function(req, res) {
	...
	pool.query('INSERT INTO ui_data (fname, lname, email, username, password) VALUES($1, $2, $3, $4, $5);', [fname, lname, email, username, dbString],
	function(err, result) {
		if(err) {
			console.log(req.body);
...
app.post('/login', function(req, res) {
	...
	pool.query('SELECT * FROM ui_data WHERE username= $1;', [username], function(err, result) {
		if(err) {
			res.status(500).send(err.toString());
		}
		else {
...
app.get('/:pageName', function(req, res) {
	...
	if(pageName=== 'about') {
		...
		pool.query("SELECT title, author FROM ui_articles", function(err, result) {
			if(err) {
				res.status(500).send(err.toString());
			}
			else {
...
var port = 5555; // Use 8080 for local development because you might already have apache running on 80
